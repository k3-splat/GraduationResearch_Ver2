"""
=============================================================================
Bidirectional Predictive Coding (bPC) Network - MNIST Training & Generation
=============================================================================

【このプログラムを実行すると何ができるか】
このスクリプトは，従来手法である双方向予測符号化（Bidirectional Predictive Coding: bPC）モデルを
用いて，MNISTデータセット（手書き数字）の画像分類と画像生成を行うPyTorchプログラムである．

プログラムを実行すると，以下のプロセスが順に実行され，結果がファイルとして出力される．

1. ハイパーパラメータ探索（Phase 1 / オプション機能）
   - 設定(`run_search`)をTrueにすると，Optunaを用いて最適な学習率，活性化関数，
     モメンタムなどのパラメータの自動探索を行う．
   - オプション(`run_search = False`)ではスキップされ，固定パラメータで実験が進行する．

2. モデルの学習とパフォーマンス計測（Phase 2）
   - MNISTの画像を使って，予測符号化のアルゴリズム（生成誤差と識別誤差の最小化）で
     ニューラルネットワークを学習させる．
   - 分類精度（Accuracy）の計測だけでなく，計算コスト（FPO: 浮動小数点演算数）や
     データ移動量（DM: Data Movement）も算出し，ハードウェア効率の観点からも評価を行う．
   - 学習中の層ごとの誤差推移を記録し，グラフとして保存．

3. ラベルからのトップダウン画像生成（Phase 3）
   - 学習完了後，ネットワークのフィードバック結合（生成経路）を利用．
   - 数字の「0」〜「9」という正解ラベル（ターゲット）だけを最上位層に与え，
     そこから逆方向に推論を行うことで，手書き数字の画像を再構成（生成）して保存する．

【出力されるファイル（logs/ ディレクトリ内）】
- bPC_log_[日時].txt        : 実行時のコンソール出力ログ
- bPC_metrics_[日時].csv    : エポックごとの精度と計算コスト（FPO/DM）の推移データ
- error_trends_[日時].png   : 学習中の層別誤差（Generative / Discriminative）推移グラフ
- generated_digits_[日時].png : クラスラベルから生成された0〜9の手書き数字画像
=============================================================================
"""
...
def run_fixed_epoch_training(best_params, train_loader, test_loader, csv_path, error_fig_path):
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"Epoch {epoch:02d} | Acc: {acc:.2f}% | FPO: {total_fpo/1e9:.1f}G | DM: {total_dm/1e9:.1f}GB")
        
        with open(csv_path, 'a', newline='') as f:
            csv.writer(f).writerow([epoch, acc, total_fpo/1e9, total_dm/1e9, time.time()-start_time])
