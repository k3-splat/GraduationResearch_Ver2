...
COMMON_CONFIG = {
    ...
    # -----------------------------------------------------------
    # [制御フラグ]
    # run_search: Trueなら探索を実行、Falseなら下記fixed_paramsを使用
    # use_feedforward_init: Trueならフィードフォワードで初期化、Falseなら0で初期化
    # -----------------------------------------------------------
    'run_search': False, 
...
class bPC_Net(nn.Module):
    ...
    def forward_zeros(self, x_in):
        ...
        for layer in self.layers:
            ...
            zeros = torch.zeros(batch_size, out_dim, device=x_in.device)
            activities.append(zeros)
        return activities
...
def run_validation(model, loader, device, params):
    ...
    total = 0
    # 初期化メソッドの選択
    use_init = COMMON_CONFIG['use_feedforward_init']
    ...
    for images, labels in loader:
        ...
        if use_init:
            ...
            activities = model.forward_zeros(images)
...
def run_fixed_epoch_training(best_params, csv_path):
    ...
    print(f"Initialization: {'Feedforward' if use_init else 'Zeros'}")
    print("="*60)
