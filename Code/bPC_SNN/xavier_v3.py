...
CONFIG = {
    ...
    'gamma_m': 1.0,
    'R_m' : 1.0,
    'alpha_u' : 0.0001,   # 【修正】学習率を下げて過学習を抑制 (0.0005 -> 0.0001)
    'alpha_gen' : 1.0,
    'alpha_disc' : 1.0,
    'thresh': 0.4,
    'batch_size': 64,
    'epochs': 20,         # 学習率を下げた分、エポック数を少し増やす
    'device': 'cuda' if torch.cuda.is_available() else 'cpu',
    'save_dir': './results/bPC_SNN',
    'max_freq': 3000.0,
    'dropout_rate': 0.2,  # 【追加】ドロップアウト率 (20%のニューロンを無効化)
    'weight_decay': 1e-4  # 【追加】重み減衰係数
}
...
class bPC_SNNLayer(nn.Module):
    ...
    def update_state(self, total_input_current):
        ...
        # 【追加】Dropout実装 (学習時かつデータ層でない場合)
        if self.training and not self.is_data_layer:
             # rand_like は [0, 1) の一様乱数。dropout_rateより大きい場合のみ 1 (通過)
...
class bPC_SNN(nn.Module):
        ...
        if y_target is not None: # Training
            for i, layer in enumerate(self.layers):
                ...
                if i == 0:
                    ...
                    # Label Layer (Training) - Driven by bottom-up V
    ...
    def manual_weight_update(self, x_data, y_target=None):
        ...
        wd = self.config['weight_decay'] # Weight Decay
...
def run_experiment(dataset_name='MNIST'):
    print(f"\n=== Running bPC-SNN on {dataset_name} (Fixed v3: Dropout & Decay) ===")
