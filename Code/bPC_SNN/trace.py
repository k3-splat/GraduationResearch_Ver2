        ...
        if y_target is not None:
            ...
            for i, layer in enumerate(self.layers):
                ...
                if i > 0 and i < len(self.layers) - 1:
                    ...
                    pass # Data layer (Input) is fixed
                
                else:
                    # Last layer (Target) is fixed during training
                    pass
            ...
            for i, layer in enumerate(self.layers):
                if i == 0:
                    x_upper = self.layers[i].x # should use trace?
                    # ここではInput(trace)とPrediction(trace)の比較
                    # x_dataはimgs_rate(trace相当)
                    ...
                    z_disc_pred = torch.matmul(x_lower, self.V[i-1].t())
                    # 教師信号との誤差
                    layer.e_disc = alpha_disc * (y_target - z_disc_pred) # y_targetはone-hot
...
def run_experiment(dataset_name='MNIST'):
    ...
    with torch.no_grad():
        for epoch in range(CONFIG['epochs']):
            ...
            # ラベル層を学習モード(固定)から推論モード(LIF更新)へ切り替え
            ...
            for imgs, lbls in test_l:
                ...
                # 【修正箇所】判定にラベル層のトレース値を使用
                # 最終ステップのトレース(x)の最大値を持つクラスを予測とする
                _, pred = torch.max(model.layers[-1].x, 1)
