...
class DiffPCNetworkTorch(nn.Module):
    ...
    def apply_phase2_update(self):
        ...
        for l, lyr in enumerate(self.layers):
            ...
            self.V[l].grad = - (cfg.alpha_disc / cfg.alpha_gen) * (post_eT_gen.T @ torch.relu(pre_xT_gen)) / Bf
            self.V_bias[l].grad = - (cfg.alpha_disc / cfg.alpha_gen) * post_eT_gen.sum(dim=0, keepdim=True).T / Bf
...
def plot_batch_error_history(error_history: List[Dict], current_run_dir: str):
    """Plots the error curves per batch (step) into the specific run directory."""
...
def main(cfg: DiffPCConfig):
    ...
    print(f"Saving to: {current_run_dir}")
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        avg_sa = total_sa_train_p2 / denom_train if denom_train > 0 else 0

        print(f"Epoch {epoch:02d}: Test Acc {test_acc:.2f}% | Avg SA {avg_sa:.4f}")
    ...
    print(f"Done. Logs: {current_run_dir}")
