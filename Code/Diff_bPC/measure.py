...
@dataclass
class SpikeStats:
    """Container for spike counts and prediction errors."""
    sa_total: float = 0.0
    se_disc_total: float = 0.0
    se_gen_total: float = 0.0
    # ### 拡張箇所: 予測誤差の絶対値和を保持するフィールド ###
    err_disc_abs_sum: float = 0.0
    err_gen_abs_sum: float = 0.0
...
class DiffPCLayerTorch(nn.Module):
    ...
    def get_current_errors(self):
        """Returns mean absolute values of e_T_disc and e_T_gen."""
        with torch.no_grad():
            e_disc_abs = self.e_T_disc.abs().mean().item()
            e_gen_abs = self.e_T_gen.abs().mean().item()
        return e_disc_abs, e_gen_abs
...
def run_batch_two_phase(net: DiffPCNetworkTorch, x: torch.Tensor, y_onehot: torch.Tensor,
    ...
    for _ in range(steps_phase1): 
        ...
        # for lyr in net.layers:
            # ed, eg = lyr.get_current_errors()
            # spike_stats.err_disc_abs_sum += ed
            # spike_stats.err_gen_abs_sum += eg
...
def main(cfg: DiffPCConfig):
    ...
    if cfg.random_crop_padding > 0:
        p_mode = "edge" if cfg.use_fashion_mnist else "constant"
        train_transforms_list.append(transforms.RandomCrop(28, padding=cfg.random_crop_padding, padding_mode=p_mode))
    ...
    ds_cls = datasets.FashionMNIST if cfg.use_fashion_mnist else datasets.MNIST
    train_ds = ds_cls("./data", train=True, download=True, transform=train_transform)
    test_ds  = ds_cls("./data", train=False, download=True, transform=test_transform)
    ...
    print(f"Starting run: {run_name} | Device: {device}")

    for epoch in range(1, cfg.epochs + 1):
        # Training
        total_sa_train, total_err_disc_train, total_err_gen_train = 0.0, 0.0, 0.0
        train_steps_per_sample = (cfg.t_init_cycles + cfg.phase2_cycles) * cfg.lt_n

        for images, labels in train_loader:
            _, stats = run_batch_two_phase(net, to_vec(images), to_onehot(labels), cfg, l_t_spec, y_phase2_spec)
            total_sa_train += stats.sa_total
            # ### 拡張箇所: 学習誤差の累積 ###
            total_err_disc_train += stats.err_disc_abs_sum
            total_err_gen_train += stats.err_gen_abs_sum

        # Evaluation
        train_correct, train_total = 0, 0
        test_correct, test_total = 0, 0
        total_err_disc_test, total_err_gen_test = 0.0, 0.0
        test_steps_per_sample = cfg.t_init_cycles * cfg.lt_n
        ...
        with torch.no_grad():
            ...
            for images, labels in test_loader:
                ...
                # ### 拡張箇所: テスト誤差の累積 ###
                total_err_disc_test += stats.err_disc_abs_sum
                total_err_gen_test += stats.err_gen_abs_sum

        train_acc = 100.0 * train_correct / train_total
        test_acc  = 100.0 * test_correct  / test_total

        # ### 拡張箇所: 各種平均の算出 ###
        denom_train = len(train_loader) * train_steps_per_sample * num_layers
        denom_test = len(test_loader) * test_steps_per_sample * num_layers
        
        avg_err_disc_train = total_err_disc_train / denom_train
        avg_err_gen_train = total_err_gen_train / denom_train
        avg_err_disc_test = total_err_disc_test / denom_test
        avg_err_gen_test = total_err_gen_test / denom_test

        print(f"Epoch {epoch:02d}: train acc {train_acc:.2f}% | test acc {test_acc:.2f}%")
        print(f"  [Train Error] Disc: {avg_err_disc_train:.6f}, Gen: {avg_err_gen_train:.6f}")
        print(f"  [Test Error ] Disc: {avg_err_disc_test:.6f}, Gen: {avg_err_gen_test:.6f}")

        run_results.append({
            "epoch": epoch,
            "train_acc": train_acc,
            "test_acc": test_acc,
            "avg_err_disc_train": avg_err_disc_train,
            "avg_err_gen_train": avg_err_gen_train,
            "avg_err_disc_test": avg_err_disc_test,
            "avg_err_gen_test": avg_err_gen_test,
        })
...
if __name__ == "__main__":
    cfg = DiffPCConfig(
        ...
        clip_grad_norm=1.0,
        seed=2,
        run_name="mnist_400h_with_errors",
        device="cuda:0"
    )
    main(cfg)