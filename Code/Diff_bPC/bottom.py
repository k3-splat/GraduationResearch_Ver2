...
class LTCyclicPhase(LTBaseScheduler):
    ...
    def get_l_t(self, sample_step: int, train: bool) -> float:
        ...
        return max(val, self.lt_min)  # <--- 下限で固定
...
def main(cfg: DiffPCConfig):
    ...
    l_t_spec = {
        ...
        "args": {"m": cfg.lt_m, "n": cfg.lt_n, "a": cfg.lt_a, "lt_min": cfg.lt_min} # <--- lt_minを注入
    }
    y_phase2_spec = {
        "type": "on_cycle_start",
...
def debug_lt(cfg: DiffPCConfig, seq_len: Optional[int] = None):
    if seq_len is None:
        seq_len = 2 * cfg.lt_n
    print("\n--- Running l_t Scheduler Sanity Check with lt_min ---")
    sched = get_l_t_scheduler(cfg.lt_scheduler_type, {"m": cfg.lt_m, "n": cfg.lt_n, "a": cfg.lt_a, "lt_min": cfg.lt_min})
    sched.begin_phase(0, seq_len * 5, a=cfg.lt_a)
    
    for t in range(seq_len):
        lt_now = sched.get_l_t(t, True)
        print(f"t={t:02d} | l_t(t): {lt_now: >9.6f}")
...
if __name__ == "__main__":
    cfg = DiffPCConfig(
        ...
        lt_m=7,
        lt_n=10,
        lt_a=1.0,         # 減衰率を低く設定してテスト
        lt_min=0.0625,       # <--- 下限テスト用: 0.5を下回ると固定される
