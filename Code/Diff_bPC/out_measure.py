...
def main(cfg: DiffPCConfig):
    ...
    # ### ANALYSIS ADDITION: History container ###
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        # ### ANALYSIS ADDITION: Container for x_T samples ###
        xt_samples = []
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            # ### ANALYSIS ADDITION: Sample x_T every 20 batches to avoid OOM ###
            if i % 20 == 0:
                # net.layers[0] is the hidden layer (since layer_dims is [Input, Hidden, Output])
                xt_samples.append(net.layers[0].x_T.detach().cpu())

        # ### ANALYSIS ADDITION: Statistics and Histogram ###
        all_xt = torch.cat(xt_samples, dim=0)
        mean_act = all_xt.abs().mean().item()
        sparsity = (all_xt.abs() < 0.01).float().mean().item()
        
        # Save Histogram
        plt.figure(figsize=(6, 4))
        plt.hist(all_xt.flatten().numpy(), bins=100, alpha=0.7, color='purple', density=True, range=(-2, 6))
        plt.yscale('log')
        plt.title(f"x_T Distribution (Epoch {epoch})")
        plt.xlabel("Value")
        plt.tight_layout()
        plt.savefig(os.path.join(current_run_dir, f"dist_epoch_{epoch:03d}.png"))
        plt.close()

        # Update History
        history['epoch'].append(epoch)
        history['sparsity'].append(sparsity)
        history['mean_activity'].append(mean_act)
        ...
        history['accuracy'].append(test_acc) # Add to history
        
        avg_sa = total_sa_train_p2 / (total_neurons * len(train_ds))

        print(f"Epoch {epoch:02d}: Test Acc {test_acc:.2f}% | Avg Spike {avg_sa:.4f} | Sparsity {sparsity:.3f} | Mean |x| {mean_act:.3f}")
        ...
        run_results.append({
            ...
            "sparsity": sparsity, "mean_activity": mean_act
        })

    # Save data
    plot_batch_error_history(global_batch_error_history, current_run_dir)
    
    # ### ANALYSIS ADDITION: Final Summary Plot ###
    plt.figure(figsize=(15, 4))
    
    # Accuracy
    plt.subplot(1, 3, 1)
    plt.plot(history['epoch'], history['accuracy'], 'o-')
    plt.title('Test Accuracy')
    plt.xlabel('Epoch'); plt.ylabel('%'); plt.grid(True)
    
    # Sparsity
    plt.subplot(1, 3, 2)
    plt.plot(history['epoch'], history['sparsity'], 'o-', color='green')
    plt.title('Sparsity (|x| < 0.01)')
    plt.xlabel('Epoch'); plt.grid(True)
    
    # Mean Activity
    plt.subplot(1, 3, 3)
    plt.plot(history['epoch'], history['mean_activity'], 'o-', color='orange')
    plt.title('Mean |x_T|')
    plt.xlabel('Epoch'); plt.grid(True)
    
    plt.tight_layout()
    plt.savefig(os.path.join(current_run_dir, "training_summary_metrics.png"))
    plt.close()
