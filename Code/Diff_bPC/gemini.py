...
class DiffPCConfig:
    ...
    # Thresholds (keep low to allow learning signals)
    lt_min_a: float = 0.0
    ...
    # Coefficients for Neural Dynamics (Inference)
    # alpha_gen can be small to not disturb classification, BUT V-update must be strong.
    alpha_disc: float = 1.0 
    alpha_gen: float = 0.1 

    # Learning Rates
    pc_lr: float = 5e-4
    # Multiplier for generative weights (V) to compensate for weak alpha_gen or high dimensionality
    gen_lr_scale: float = 10.0 
...
class DiffPCLayerTorch(nn.Module):
    ...
    def step(self, clamp_status: bool, data_in: Optional[torch.Tensor],
        ...
        # Integrate spikes to update predictions
        self.x_F_disc.add_(self.s_in_disc * l_t_prev)
        self.x_F_gen.add_(self.s_in_gen * l_t_prev)
        
        # Calculate Errors (Raw errors, not suppressed)
        self.e_T_disc = (self.x_T - self.x_F_disc)
        self.e_T_gen = (self.x_T - self.x_F_gen)
        
        # Initial phase suppression (optional)
        if hasattr(self, 'ff_init_duration') and sample_step < self.ff_init_duration and bottomup_mask:
            self.x_F_disc.zero_(); self.e_T_disc.zero_()
        if hasattr(self, 'ff_init_duration') and sample_step < self.ff_init_duration and topdown_mask:
            self.x_F_gen.zero_(); self.e_T_gen.zero_()

        # Integrate incoming error spikes
        self.e_in_disc.add_(e_in_disc.to(self.device) * l_t_prev)
        self.e_in_gen.add_(e_in_gen.to(self.device) * l_t_prev)

        # Update Target Activity (Dynamics)
        # Here we use alpha scaling to balance the *inference*, but we don't suppress the errors themselves.
        # Ideally, e_in should be normalized by sqrt(dim) if not already done by weights.
        if not clamp_status:
            total_error_drive = (
                - self.alpha_disc * self.e_T_disc 
                - self.alpha_gen * self.e_T_gen 
                + (self.x_T > 0).float() * (self.alpha_disc * self.e_in_disc + self.alpha_gen * self.e_in_gen)
            )
            self.x_T.add_(self.y * total_error_drive)
        ...
        # Note: We spike based on e_T_disc/gen which are UNWEIGHTED by alpha.
        # This allows the generative error to be transmitted even if alpha_gen is small in the x_T update.
        ...
        if hasattr(self, 'ff_init_duration') and sample_step < self.ff_init_duration:
            s_e_disc_new.zero_(); s_e_gen_new.zero_()
...
class DiffPCNetworkTorch(nn.Module):
    def __init__(self, cfg: DiffPCConfig, l_t_scheduler_spec: Dict, y_scheduler_spec: Dict, device: str):
        ...
        # W: Discriminative Weights (layer i -> layer i+1) or (i+1 -> i)? 
        # Standard: W[i] maps layer i input to layer i+1 output (Forward/Bottom-up prediction)
        # Dimensions: [layer_dims[i+1], layer_dims[i]]
        ...
        # V: Generative Weights (layer i+1 -> layer i)
        # Dimensions: [layer_dims[i], layer_dims[i+1]]
        ...
        # This is CRITICAL: Generative weights need stronger learning signals if alpha_gen is small
        disc_params = list(self.W) + list(self.W_bias)
        gen_params  = list(self.V) + list(self.V_bias)

        if cfg.use_adamw:
            self.optimizer = torch.optim.AdamW([
                {'params': disc_params, 'lr': cfg.pc_lr},
                {'params': gen_params,  'lr': cfg.pc_lr * cfg.gen_lr_scale} 
            ], betas=cfg.adamw_betas, eps=cfg.adamw_eps, weight_decay=cfg.adamw_weight_decay)
        else:
    ...
    def _build_s_in_and_e_in(self):
        ...
        # Predictions (Forward s_in_disc, Backward s_in_gen)
        prev_s_disc = self.input_driver.s_A_prev
        prev_s_gen = self.layers[0].s_A_prev # Start from first hidden layer for generation down to input

        # s_in_disc: Input -> L1 -> L2 ...
        for l in range(len(self.W)):
            # W[l] connects l to l+1. prev_s_disc is at l.
            s_in_disc_l = prev_s_disc @ self.W[l].T
            if bias_spike != 0.0: s_in_disc_l.add_(self.W_bias[l].T)
            s_in_disc.append(s_in_disc_l)
            prev_s_disc = self.layers[l].s_A_prev
        
        # s_in_gen: L_last -> ... -> Input
        # Note: self.V[0] connects L1 (hidden) to Input. self.V[1] connects L2 to L1.
        # We need s_in_gen for Input layer first? 
        # input_driver needs s_in_gen from layer[0].
        # V[0] is (input_dim, L1_dim). 
        # layer[0].s_A_prev is (B, L1_dim).
        # s_in_gen_0 = layer[0].s_A_prev @ V[0].T -> (B, input_dim)
        
        # Input driver gen input
        s_in_gen_0 = self.layers[0].s_A_prev @ self.V[0].T
        if bias_spike != 0.0: s_in_gen_0.add_(self.V_bias[0].T)
        s_in_gen.append(s_in_gen_0) # For input driver

        for l in range(len(self.layers) - 1):
             # Layer l needs gen input from l+1
             # V[l+1] connects l+1 to l.
             s_gen_l = self.layers[l+1].s_A_prev @ self.V[l+1].T
             if bias_spike != 0.0: s_gen_l.add_(self.V_bias[l+1].T)
             s_in_gen.append(s_gen_l)
        
        # Top layer receives 0 gen input (no layer above)
        # Handled in one_time_step by appending zero tensor
        
        # Errors
        # e_in_disc: from layer l+1 to l (Standard PC feedback)
        # e_in_gen: from layer l-1 to l (Inverse PC feedback)
        
        # e_in_disc for Input Driver? No, input driver only sends x_F, doesn't receive disc error from *below*.
        # Wait, e_in_disc comes from layer above. 
        # Input Driver e_in_disc = layers[0].s_e_disc @ W[0]
        e_in_disc_0 = self.layers[0].s_e_disc_prev @ self.W[0]
        e_in_disc.append(e_in_disc_0)

        for l in range(len(self.layers) - 1):
             # Layer l receives disc error from l+1
             e_disc_l = self.layers[l+1].s_e_disc_prev @ self.W[l+1]
             e_in_disc.append(e_disc_l)
        # Top layer receives 0 disc error (handled in step or passed as 0)
        e_in_disc.append(torch.zeros_like(self.layers[-1].s_e_disc_prev)) # Placeholder for top

        # e_in_gen
        # Input driver gen error? No, it's at the bottom.
        # Layer 0 (L1) receives gen error from Input Driver (L0)
        e_in_gen_0 = self.input_driver.s_e_gen_prev @ self.V[0]
        e_in_gen.append(e_in_gen_0)

        for l in range(len(self.layers) - 1):
             # Layer l+1 receives gen error from layer l
             e_gen_lplus1 = self.layers[l].s_e_gen_prev @ self.V[l+1]
             e_in_gen.append(e_gen_lplus1)
    ...
    def one_time_step(self, bottomup_mask: bool = False, topdown_mask : bool = False):
        ...
        z_in = torch.zeros(B, self.input_driver.dim, device=self.device)
        ...
        # Input Driver Step
        # Disc Input: None (Raw Data). Gen Input: s_in_gen[0].
        # Disc Error In: e_in_disc[0]. Gen Error In: None (Bottom).
        self.input_driver.step(self.input_driver_clamp, self.input_driver_data, 
                               z_in, s_in_gen[0], 
                               e_in_disc[0], z_in, 
                               False, topdown_mask, sample_step_override=t)

        for i, lyr in enumerate(self.layers):
            # s_in_disc is aligned with layers (0->L1, 1->L2)
            # s_in_gen list started with input_driver, so s_in_gen[i+1] is for layer i
            # e_in_disc list started with input_driver, so e_in_disc[i+1] is for layer i
            # e_in_gen list started with layer 0, so e_in_gen[i] is for layer i
            
            gen_in = s_in_gen[i+1] if i+1 < len(s_in_gen) else z_top
            
            # e_in_disc[i] corresponds to feedback from i+1 to i?
            # In build: e_in_disc[0] is L1->Input. e_in_disc[1] is L2->L1.
            # So for layer 0, we need e_in_disc[1].
            disc_err_in = e_in_disc[i+1] if i+1 < len(e_in_disc) else z_top

            lyr.step(self._clamp_switch[i], self._data_bucket[i], 
                     s_in_disc[i], gen_in, 
                     disc_err_in, e_in_gen[i],
    ...
    def apply_phase2_update(self):
        """
        Explicit Manual Weight Update for Bidirectional PC.
        Ensures W updates minimize Discriminative Error and V updates minimize Generative Error.
        """
        ...
        # --- Discriminative Weights Update (W) ---
        # W[l] maps x[l] -> x[l+1] (prediction).
        # Error term is e_T_disc at l+1.
        # Input term is f(x_T) at l.
        # Grad = e_T_disc[l+1] * x_T[l].T
        
        # W[0]: Input(x_T) -> Layer0(e_T_disc)
        pre_xT = self.input_driver.x_T
        post_err = self.layers[0].e_T_disc
        dw = post_err.T @ ((pre_xT > 0).float() * pre_xT) # Assuming ReLu-like plasticity or just pre_xT
        # Standard PC uses pre_xT directly or f(pre_xT). Here x_T is the variable.
        if self.W[0].grad is None: self.W[0].grad = torch.zeros_like(self.W[0])
        self.W[0].grad.add_(-dw) # Gradient Descent
        
        for l in range(len(self.layers) - 1):
             pre_xT = self.layers[l].x_T
             post_err = self.layers[l+1].e_T_disc
             dw = post_err.T @ pre_xT
             if self.W[l+1].grad is None: self.W[l+1].grad = torch.zeros_like(self.W[l+1])
             self.W[l+1].grad.add_(-dw)
             
        # --- Generative Weights Update (V) ---
        # V[l] maps x[l+1] -> x[l] (prediction).
        # Error term is e_T_gen at l.
        # Input term is f(x_T) at l+1.
        
        # V[0]: Layer0(x_T) -> Input(e_T_gen)
        pre_xT_gen = self.layers[0].x_T
        post_err_gen = self.input_driver.e_T_gen
        dv = post_err_gen.T @ pre_xT_gen
        if self.V[0].grad is None: self.V[0].grad = torch.zeros_like(self.V[0])
        self.V[0].grad.add_(-dv)

        for l in range(len(self.layers) - 1):
            pre_xT_gen = self.layers[l+1].x_T
            post_err_gen = self.layers[l].e_T_gen
            dv = post_err_gen.T @ pre_xT_gen
            if self.V[l+1].grad is None: self.V[l+1].grad = torch.zeros_like(self.V[l+1])
            self.V[l+1].grad.add_(-dv)

        # Apply updates
        # Note: scale is handled by optimizer param_groups lr
        self.optimizer.step()
...
def visualize_generated_digits(net: DiffPCNetworkTorch, cfg: DiffPCConfig, 
    ...
    generated_data = infer_batch_backward_phase2(net, y_onehot, cfg, l_t_spec, y_phase2_spec)
    ...
    save_path = os.path.join(current_run_dir, f"gen_epoch_{epoch:03d}.png")
    save_image(gen_imgs, save_path, nrow=10)


def plot_batch_error_history(error_history: List[Dict], current_run_dir: str):
    """Plots the error curves per batch into the specific run directory."""
    ...
    for i in range(num_layers):
        plt.plot(steps, layers_disc_hist[i], linewidth=0.5, label=f'Layer {i+1} Disc')
    ...
    plt.subplot(1, 2, 2)
    plt.plot(steps, input_gen_hist, linewidth=0.5, linestyle='--', label='Input (L0) Gen')
    for i in range(num_layers):
        plt.plot(steps, layers_gen_hist[i], linewidth=0.5, label=f'Layer {i+1} Gen')
    ...
    save_path = os.path.join(current_run_dir, "batch_error_curves.png")
    plt.tight_layout()
...
def main(cfg: DiffPCConfig):
    ...
    base_name = cfg.run_name or "diffpc_run"
    ...
    test_transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((mean,), (std,)) if cfg.normalize else transforms.Lambda(lambda x: x)
    ])
    ...
    print(f"Results saved to: {current_run_dir}")

    for epoch in range(1, cfg.epochs + 1):
        total_sa_train_p2 = 0.0
        total_samples = 0
        ...
        avg_sa = total_sa_train_p2 / (total_neurons * len(train_ds))

        print(f"Epoch {epoch:02d}: Test Acc {test_acc:.2f}% | Avg Spike {avg_sa:.4f}")

        # Visualize
        visualize_generated_digits(net, cfg, l_t_spec, y_phase2_spec, epoch, current_run_dir)

        run_results.append({
            "epoch": epoch, "test_acc": test_acc, "avg_sa": avg_sa
        })

    # Save data
    plot_batch_error_history(global_batch_error_history, current_run_dir)
    with open(log_path, "w") as f:
        json.dump({"config": asdict(cfg), "results": run_results}, f, indent=4)
    print(f"Done. Files in {current_run_dir}")
