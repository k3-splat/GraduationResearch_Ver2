...
class LTCyclicPhase(LTBaseScheduler):
    # MODIFICATION: Removed lt_min from scheduler (handled in Layer)
    ...
    def get_l_t(self, sample_step: int, train: bool) -> float:
        ...
        # Returns raw decay value; min clamping happens in Layer
...
class DiffPCLayerTorch(nn.Module):
    def __init__(self, **kwargs):
        ...
        # Ensure default values for new mins if not passed (backward compatibility)
    ...
    def step(self, clamp_status: bool, data_in: Optional[torch.Tensor],
        ...
        # Scheduler returns the raw decayed value (without min clamping)
        ...
        self.l_t = l_t_cur_base # Storing base for reference
        self.y   = self.y_scheduler.get_y(sample_step, self.learning_weights)

        # --- MODIFICATION: Apply specific mins for integration scale ---
        # Note: Using previous step's threshold for integration is standard in this arch.
        # We use the activity threshold (lt_min_a) as the default scaling for integration if ambiguous,
        # but technically x_F integrates spikes which were generated with specific thresholds.
        # For simplicity and stability, we often use the base l_t or l_t_a for integration scaling.
        # Here we follow the pattern: use the effective threshold that generated the spike?
        # No, usually integration is scaled by the current 'membrane leak' equivalent.
        # We will use l_t_prev_base clipped by lt_min_a for general integration scaling to keep it simple,
        # or just use the base if we want the leak to go to zero. 
        # Let's use max(base, lt_min_a) as the "time constant" equivalent for updates.
        l_t_prev = max(l_t_prev_base, self.lt_min_a) 
        l_t_cur_a = max(l_t_cur_base, self.lt_min_a)
        
        # For error spikes, we can use their specific thresholds or the general one. 
        # Standard implementation uses one global l_t. We will use the specific ones for *generation* of spikes,
        # and l_t_prev (activity based) for *integration* to maintain dimensional consistency.
        ...
        # --- MODIFICATION: Variable Thresholds for Spike Generation ---
        
        # 1. Activity Spikes (s_A)
        # Use lt_min_a
        ...
        # 2. Error Spikes (s_e)
        # Use lt_min_e_disc and lt_min_e_gen
...
class DiffPCNetworkTorch(nn.Module):
    def __init__(self, cfg: DiffPCConfig, l_t_scheduler_spec: Dict, y_scheduler_spec: Dict, device: str):
        ...
        # --- MODIFICATION: Pass new threshold mins to layer args ---
        layer_args = {
...
def run_batch_two_phase(net: DiffPCNetworkTorch, x: torch.Tensor, y_onehot: torch.Tensor,
    ...
    # Measure Errors (Sum of Absolute Error) once at the end of phase 2
    # ----------------------------------------------------
    batch_error_stats = {
...
def plot_batch_error_history(error_history: List[Dict], output_dir: str = "plots"):
    ...
    plt.title('Discriminative Error Snapshot (end of Phase 2)')
    plt.xlabel('Global Batch Step')
    plt.ylabel('Avg |e_T_disc| (final)')
    plt.legend()
    ...
    plt.title('Generative Error Snapshot (end of Phase 2)')
    plt.xlabel('Global Batch Step')
    plt.ylabel('Avg |e_T_gen| (final)')
    plt.legend()
    plt.grid(True, alpha=0.3)
...
def main(cfg: DiffPCConfig):
    ...
    # MODIFICATION: Removed 'lt_min' from args here, as it's now handled in Layer/Config
    l_t_spec = {
